...
canvas.onmousemove = function drawIfPressed(e) {
   ...
   function autoDraw() {
       ...
       var drawing = setInterval(() => {
           ...
           var currentLine = lineSizes.shift();
           var currentColors = colorses.shift();
           ...
           if (xu.length <= 0 && yu.length <= 0) {
               ...
               if (currentX == undefined && currentY == undefined && currentDY == undefined && currentDX == undefined && currentLine == undefined && currentColors == undefined) {
                   ctx.beginPath();
               } else {
                   drawLine(currentX, currentY, currentDX, currentDY, currentLine, currentColors);
               }
           }
       }, 1);

   }
    function drawLine(xu, yu, dxu, dyu, line, colore) {
        ctx.lineWidth = line;
        ctx.lineJoin = 'round';
        ctx.lineCap = 'round';
        ctx.moveTo(xu, yu);
        ctx.lineTo(xu - dxu, yu - dyu);
        ctx.stroke();
        ctx.closePath();
